...
def main():
    parser = argparse.ArgumentParser(description='Comparison by models (ParT, Particle Net) for 10M and - JetClass full using normalized discriminant')
    ...
    parser.add_argument('--output-dir', type=str, default='graficos_comparativos_v3/JetClass/model_comparison')
    parser.add_argument('--title', type=str, default='JetClass full 10M- Model comparison (Normalized Discriminant)')
    ...
    def bar_plot(metric_key, ylabel, filename):
        ...
        vals = [res[s]['10M'][metric_key] for s in models]
        bars1 = plt.bar(x - width/2, vals, width)
        
        # Add value labels on bars
        for bars, vals in [(bars1, vals)]:
    ...
    bar_plot('roc_auc_macro_ovo', 'ROC AUC (macro OVO)', 'roc_auc_macro_ovo_by_model.png')
    bar_plot('avg_precision_macro_ovr', 'Average Precision (macro OVR)', 'avg_precision_macro_ovr_by_model.png')
    bar_plot('pr_auc_macro_signal_qcd', 'PR AUC (signals vs QCD, macro)', 'pr_auc_macro_signal_qcd_by_model.png')
    bar_plot('f1_macro_signal_qcd', 'F1 (signals vs QCD, macro)', 'f1_macro_signal_qcd_by_model.png')
