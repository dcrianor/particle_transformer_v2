...
def main():
    parser = argparse.ArgumentParser(description='Comparison by features subsets (kin, kinpid, full) for ParT y PN - JetClass 10M using normalized discriminant')
    parser.add_argument('--part_kin', type=str, default='predictions/JetClass/Pythia/kin/ParT/10M/*_pred_*.root')
    parser.add_argument('--part_kinpid', type=str, default='predictions/JetClass/Pythia/kinpid/ParT/10M/*_pred_*.root')
    parser.add_argument('--part_full', type=str, default='predictions/JetClass/Pythia/full/ParT/10M/*_pred_*.root')
    parser.add_argument('--pn_kin', type=str, default='predictions/JetClass/Pythia/kin/PN/10M/*_pred_*.root')
    parser.add_argument('--pn_kinpid', type=str, default='predictions/JetClass/Pythia/kinpid/PN/10M/*_pred_*.root')
    parser.add_argument('--pn_full', type=str, default='predictions/JetClass/Pythia/full/PN/10M/*_pred_*.root')
    parser.add_argument('--score-prefix', type=str, default='score_')
    parser.add_argument('--output-dir', type=str, default='graficos_comparativos_v3/JetClass/subset_features_comparison')
    parser.add_argument('--title', type=str, default='JetClass 10M - Feature subset comparison (Normalized Discriminant)')
    ...
    res = {
        'ParT': {
            'kin': summarize_group('ParT_kin_10M', [args.part_kin], score_prefix=args.score_prefix, verbose=args.verbose),
            'kinpid': summarize_group('ParT_kinpid_10M', [args.part_kinpid], score_prefix=args.score_prefix, verbose=args.verbose),
            'full': summarize_group('ParT_full_10M', [args.part_full], score_prefix=args.score_prefix, verbose=args.verbose),
        },
        'PN': {
            'kin': summarize_group('PN_kin_10M', [args.pn_kin], score_prefix=args.score_prefix, verbose=args.verbose),
            'kinpid': summarize_group('PN_kinpid_10M', [args.pn_kinpid], score_prefix=args.score_prefix, verbose=args.verbose),
            'full': summarize_group('PN_full_10M', [args.pn_full], score_prefix=args.score_prefix, verbose=args.verbose),
        }
    }

    # CSV table
    csv_path = os.path.join(args.output_dir, 'macro_summary_by_subset_features_discriminant.csv')
    with open(csv_path, 'w') as f:
        ...
        for model in ['ParT', 'PN']:
            for feat in ['kin', 'kinpid', 'full']:
                r = res[model][feat]
                f.write(f"{model},{feat},{r['num_samples']},{r['roc_auc_macro_ovo']:.6f},{r['avg_precision_macro_ovr']:.6f},{r['pr_auc_macro_signal_qcd']:.6f},{r['f1_macro_signal_qcd']:.6f}\n")

    # Plots per metric
    feats = ['kin', 'kinpid', 'full']
    x = np.arange(len(feats))
    width = 0.35

    def bar_plot(metric_key, ylabel, filename):
        plt.figure(figsize=(8, 5))
        vals_part = [res['ParT'][s][metric_key] for s in feats]
        vals_pn = [res['PN'][s][metric_key] for s in feats]
    ...
    bar_plot('roc_auc_macro_ovo', 'ROC AUC (macro OVO)', 'roc_auc_macro_ovo_by_subset_features.png')
    bar_plot('avg_precision_macro_ovr', 'Average Precision (macro OVR)', 'avg_precision_macro_ovr_by_subset_features.png')
    bar_plot('pr_auc_macro_signal_qcd', 'PR AUC (signals vs QCD, macro)', 'pr_auc_macro_signal_qcd_by_subset_features.png')
    bar_plot('f1_macro_signal_qcd', 'F1 (signals vs QCD, macro)', 'f1_macro_signal_qcd_by_subset_features.png')

    print('[OK] Saved:', csv_path)
    print('[OK] Evaluation completed using normalized discriminant for signals vs QCD metrics')
