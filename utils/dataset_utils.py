...
def _download(url, fname, chunk_size=4096,  progress_callback=None):
...
def get_file(origin=None,
    ...
    if download:
            ...
            with tqdm(unit='B', unit_scale=True, unit_divisor=1024, desc=f"Downloading {fname}") as pbar:
                try:
                    try:
                        _download(origin, fpath, progress_callback=pbar.update)
